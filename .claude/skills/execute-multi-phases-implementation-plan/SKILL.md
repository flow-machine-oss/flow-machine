...
This skill implements features by executing multi-phase implementation plans sequentially, spawning `implementation-executor` agents for each phase.
...
5. **Commit the phase changes:**
   ...
   After each successfully completed phase, use the Skill tool to invoke the `commit` skill:
   ...
   ```
   feat(workflow): implement Phase N - [Phase Name]
   ...
   This ensures each phase's changes are tracked separately in version control.

   After committing, update the status:

   ```
   ✓ Phase N: [Phase Name] — COMPLETED & COMMITTED
     - Files changed: [count]
     - Tests added: [count]
     - Summary: @context/IMPLEMENTATION_SUMMARY_PHASE_N_[NAME].md
...
6. **Proceed or halt:**
   - If phase completed and committed successfully, proceed to the next phase
...
═══════════════════════════════════════════════════════════════
✓ Phase 1: Foundation — COMPLETED & COMMITTED
✓ Phase 2: Domain — COMPLETED & COMMITTED
