...
} from "@/v2/core/domain/workflow/definition/crud-repository";
import { WorkflowDefinitionEntity } from "@/v2/core/domain/workflow/definition/entity";
import type { WorkflowDefinitionMongoModel } from "@/v2/infra/mongo/workflow/definition/model";

class WorkflowDefinitionMongoCrudRepository
  implements WorkflowDefinitionCrudRepository
{
  async findMany(
    input: z.infer<
      typeof workflowDefinitionCrudRepositoryInputSchema.findMany
    >,
  ) {
    const { ctx } = input;
  ...
  async findOne(
    input: z.infer<
      typeof workflowDefinitionCrudRepositoryInputSchema.findOne
    >,
  ) {
    const { ctx, id } = input;
  ...
  async insert(
    input: z.infer<
      typeof workflowDefinitionCrudRepositoryInputSchema.insert
    >,
  ) {
  ...
  async update(
    input: z.infer<
      typeof workflowDefinitionCrudRepositoryInputSchema.update
    >,
  ) {
  ...
  async delete(
    input: z.infer<
      typeof workflowDefinitionCrudRepositoryInputSchema.delete
    >,
  ) {
    const { ctx, id } = input;
  ...
  async #getCollection({ mongoDb }: MongoCtx) {
    const collection =
      mongoDb.collection<WorkflowDefinitionMongoModel>("workflow-definition");
