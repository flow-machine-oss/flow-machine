...
} from "@/v2/api/module/workflow/definition/v1/http-dto";
...
import type { WorkflowDefinitionEntity } from "@/v2/core/domain/workflow/definition/entity";
        ...
        r
          .post(
            ...
            async ({ body, ctx, tenant }) => {
              const result =
                await this.#workflowDefinitionCrudService.create({
                  ctx: { ...ctx, tenant },
                  payload: body,
                });
              if (result.isErr()) {
          ...
          .delete(
            ...
            async ({ ctx, tenant, params }) => {
              const result =
                await this.#workflowDefinitionCrudService.delete({
                  ctx: { ...ctx, tenant },
                  payload: { id: params.id },
                });
              if (result.isErr()) {
